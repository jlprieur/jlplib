...
int JLP_WRITEIMAG(float *array, int *nx1, int *ny1, int *idim1,
...
int JLP_D_WRITEIMAG(double *d_array, int *nx1, int *ny1, int *idim1,
                  char *filename, char *comments)
{
char jlp_descr[1]; 
int dflag, istatus, out_type;
...
* out_type = 1 : real (-32) 
**********************************************************************/
int JLP_WRFITS(float *array, int *nx1, int *ny1, int *idim1,
               char *filename, char *comments, char *jlp_descr, int *dflag,
               int *istatus, int *out_type)
{
char     author[20], buf1[40], buf2[40];
...
/* JLP99: overwrite file if already there: */
   if(istat) {
    fprintf(stderr, "File already here: please delete it first\n");
    exit(-1);
/*
    printf("File already here: I delete it and overwrite it \n");
...
/* "AUTHOR  " */
  ...
  strcpy(buf1, "Jean-Louis Prieur");
  strcpy(buf2, "With cfitsio (Version May 2008)");
  fits_update_key(fptr, TSTRING, author, buf1, buf2, &istat);
  if(istat)
   {
   fits_read_errmsg(err_message);
