...
for(i = 0; i < nn; i++)
{
  ...
  meanx = meanx / (double)((float)nn);
  meany = meany / (double)((float)nn);
  printf(" mean: (should be 0.5 and %f) %f %f \n", mm, meanx,meany);
 
  sigmax = meansqx / (double)((float)nn) - meanx * meanx;
  sigmay = meansqy / (double)((float)nn) - meany * meany;
  sigmax = sqrt(sigmax);
  sigmay = sqrt(sigmay);
...
float r1, r2, val1, val2, fac, rad2;
