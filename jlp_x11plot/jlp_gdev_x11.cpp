...
************************************************************************/
int JLP_GDev_X11::setup_device_for_curve(const char *plotdev, const char* title,
                                    int& offx, int& offy, int& axlen, 
                                    int& aylen, int& dev_width, 
                                    int& dev_height, int& TeX_flag, 
                                    int& devtype, int& landscape, int idev1) 
{
int status = 0;

// JLP 2006: I reduce axlen and aylen to leave some space for the menu:
  offx = 4000; offy = 6000; axlen = 22000; aylen = 22000;
  dev_width = 600; dev_height = 500;
...
switch(plotdev[1])
    {
/* X window for curves: ("xterm", "XTERM", "xterm_small", "XTERM_SMALL",
* "xterm_small", "XSQUARE", "xsquare", "XSQUA_SMALL" or "xsqua_small")*/
    case 'T':
    case 't':
      dev_width = 600; dev_height = 400;
      break;
    case 'S':
    case 's':
/* xsquare or XSQUARE: */
      dev_width = 400; dev_height = 400;
      break;
    default:
      devtype = 0;
      printf("setup_device_for_curve/Error: unknown X11 graphic device\n (%s is unknown for curves)\n",
             plotdev);
      status = -1;
      break;
    }

/* Possibility of a smaller window (for curves only, since pb with images): */
/* XTERM_SMALL or xterm_small */
     if(plotdev[6] == 's' || plotdev[6] == 'S') {
           dev_width = (int)((double)dev_width * 0.7); 
           dev_height = (int)((double)dev_height * 0.7); 
           }
...
* devtype1 = 1 if X11, 2 if postscript, ...
************************************************************************/
int JLP_GDev_X11::setup_device_for_image(const char *plotdev, const char *title,
...
************************************************************/
int JLP_GDev_X11::open(const char* title, int dev_width1, int dev_height1,
                  const int landscape)
{
...
if(italk > 1) {
  ...
  printf(" JLP_GDev_X11::open/Warning: This program assumes %d bits per pixel of video board (%d colors)\n", 
...
size_hints.width = Jgc0.dev_width; 
size_hints.height = Jgc0.dev_height; 
size_hints.min_width = Jgc0.dev_width; 
size_hints.min_height = Jgc0.dev_height;
size_hints.max_width = Jgc0.dev_width; 
size_hints.max_height = Jgc0.dev_height;
...
/* Select event types wanted: */
 XSelectInput(Xgc0.display,Xgc0.win,
  ...
  Xgc0.backing = XCreatePixmap(Xgc0.display, Xgc0.win, Jgc0.dev_width, 
                               Jgc0.dev_height, 
                               DefaultDepth(Xgc0.display,Xgc0.screen));
...
*
************************************************************/
int JLP_GDev_X11::create_win_gc()
{
...
* Mgc0.lwidth updated
************************************************************/
int JLP_GDev_X11::line_device(int x1, int y1, int x2, int y2, int lwidth)
{
...
// When drawing vector is filled, draw whole set of (start,end) pairs:
    if(++(Xgc0.nvec) >= Xgc0.max_nvec) JLP_GDev_X11::empty_xvec();
...
*************************************************************/
int JLP_GDev_X11::SetLineWidthAndType_to_gc(GC& MyGC, int lwidth, int ltype)
{
...
*****************************************************************/
double JLP_GDev_X11::label_device(const char *s, int xstart, int ystart, 
...
Jgc0.cwidth = 0.6 * expanded_font / g_dx;
Jgc0.cheight = 0.5 * expanded_font / g_dy;
...
/* Debug:
printf("JLP_GDev_X11::label_device: s=%s< xstart=%d ystart=%d ix=%d iy=%d slen=%d\n",
...
*/
  ...
  switch (status) {
    case BadDrawable:
     printf("JLP_GDev_X11::label_device/XDrawString/error: bad drawable\n");
     break;
    case BadMatch:
     printf("JLP_GDev_X11::label_device/XDrawString/error: bad match\n");
     break;
    case BadGC:
     printf("JLP_GDev_X11::label_device/XDrawString/error: bad graphic context\n");
     break;
    default:
     break;
    }
...
/* Read the current size of the window: 
   ...
   if((Jgc0.dev_width != window_attributes.width) ||
       (Jgc0.dev_height != window_attributes.height)){
         Jgc0.dev_width = window_attributes.width;
         Jgc0.dev_height = window_attributes.height;
...
/* Erase the exposed window: */
    XFillRectangle(Xgc0.display, Xgc0.win, Xgc0.erasegc, 0, 0, Jgc0.dev_width, 
                   Jgc0.dev_height);
...
int JLP_GDev_X11::color_ressources()
{
char argv[30];
Visual *visual1;
int depth;

strcpy(argv,"JLP_GDev_X11::color_ressources");
...
switch (visual1->c_class) {
 ...
 if(Xgc0.color_code == 0) JLP_GDev_X11::look_for_visuals();
#ifdef DEBUG
printf(" color_code = %d\n", Xgc0.color_code);
#endif
...
* for R/W colorcells: 
************************************************************************/
int JLP_GDev_X11::look_for_visuals()
{
 ...
 ****************************************************************/
int JLP_GDev_X11::load_standard_lut(int *r, int *g, int *b, int *mylut_x11,
                                   int ncolors)
{
   Visual *visual1;
...
*/
   ...
   {printf(" JLP_GDev_X11::load_standard_lut/no pseudo-color: i.e. no read/write color cells\n");
    JLP_GDev_X11::color_ressources(); 
    return(-1);
    }
...
/* If OK load a standard color map: */
   if (XGetStandardColormap(Xgc0.display, RootWindow(Xgc0.display,Xgc0.screen),
	&mycmap,XA_RGB_BEST_MAP) == 0)
      {
      printf(" JLP_GDev_X11::load_standard_lut/Error: Couldn't get RGB_BEST color map\n");
      if (XGetStandardColormap(Xgc0.display, RootWindow(Xgc0.display,Xgc0.screen),
	&mycmap,XA_RGB_DEFAULT_MAP) == 0) {
         printf(" JLP_GDev_X11::load_standard_lut/Error: Couldn't get RGB_DEFAULT color map\n");
         return(-2);
         }
      }
...
**********************************************************/
int JLP_GDev_X11::alloc_lut_device(int *mylut_x11, int& private_lut, int& ncolors)
{
register int i;
...
/* Check maximum number of colors: */
  if(ncolors > Xgc0.max_colors) 
     {printf(" JLP_GDev_X11::alloc_lut/Error: Only %d colors are allowed!\n",Xgc0.max_colors);
...
if( Xgc0.color_code == 0)
  { 
#ifdef DEBUG
  printf(" JLP_GDev_X11::alloc_lut/Warning: read/write color cells cannot be allocated with that display\n"); 
#endif
  private_lut = 0;
#if 1
  return(2);
  ...
  XSetWindowColormap(Xgc0.display, Xgc0.win, Xgc0.cmap);
   for(i=0; i < ncolors; i++)
     ...
     if(!XAllocColor(Xgc0.display,Xgc0.cmap,&myxcol[i]))
      printf("JLP_GDev_X11::alloc_lut/error allocating color #%d \n",i);
     else
...
/* jlp92 */
    {
     printf(" JLP_GDev_X11::alloc_lut/error: can't allocate %d read/write color cells!\n",
             (int)ncolors); 
...
#ifdef DEBUG
  printf(" JLP_GDev_X11::alloc_lut/ %d read/write color cells successfully allocated\n",
          (int)ncolors); 
#endif
  }
 ...
 *********************************************************************/
int JLP_GDev_X11::load_lut_device(const int *r, const int *g, const int *b, 
...
/* If rw private cells, stores rgb values in the preallocated color cells: */
  if(Xgc0.color_code == 1)
   ...
   if(XStoreColors(Xgc0.display,Xgc0.cmap,myxcol,ncolors) == 0) 
     {
     printf(" JLP_GDev_X11::load_lut/XStoreColors error when storing colors in rw cells\n"
);
...
/* Get index values of the read/only cell for mylut_x11
   ...
   for(i=0; i < ncolors; i++)
     ...
     myxcol[i].flags = DoRed | DoGreen | DoBlue;
     if(!XAllocColor(Xgc0.display,Xgc0.cmap,&myxcol[i]))
      printf("JLP_GDev_X11::load_lut/error allocating color #%d \n",i);
     else
      mylut_x11[i] = myxcol[i].pixel;
     }
   }
...
***************************************************************/
int JLP_GDev_X11::setrgbcolor(double r, double g, double b)
{
int status = 0;
...
#ifdef DEBUG
   printf("JLP_GDev_X11::setrgbcolor/line color set to r, g, b: %d %d %d \n",
...
*
***************************************************************/
int JLP_GDev_X11::wait_for_events()
{
  XEvent event;
...
/* Change of size: 
       case ConfigureNotify:
         ...
         Jgc0.dev_width = event.xconfigure.width;
         Jgc0.dev_height = event.xconfigure.height;
// Send a message and exit (TO BE CHANGED LATER...)
         printf("Window has been resized! Not handled in current version!\n");
         exit(-1);
*/
...
* JLP96
***************************************************************/
int JLP_GDev_X11::get_winlimits(double *u_x1, double *u_y1, double *u_x2, 
...
*
***************************************************************/
int JLP_GDev_X11::get_circles(double& x_cent, double& y_cent, double& diam1, 
...
*
***************************************************************/
int JLP_GDev_X11::gflush()
{

#ifdef DEBUG
  printf("JLP_GDev_X11::gflush: OK\n");
#endif

/* Empty vector buffer: */
    if(Xgc0.nvec) JLP_GDev_X11::empty_xvec();
...
*********************************************************************/
int JLP_GDev_X11::polygon(int xc, int yc, double expand, double angle, int nsides, 
                     int filled)
{
...
********************************************************************/
int JLP_GDev_X11::cursor(int& x, int& y, char *cursor_type, int& pressed_button)
{
...
// Will exit from loop since npoints != 0: 
        npoints++;
#ifdef DEBUG
      printf("JLP_GDev_X11::cursor/Pressed button: x=%d y=%d (mgo)\n", x, y);
#endif
        break;
      case MotionNotify: 
/* can get rid of all MotionNotify events in queue,
...
// For curves: 
          if(Jgc0.gdev_graphic_type == 1)
            sprintf(buffer,"%.3g %.3g",x_user,y_user);
// For images: 
          else 
              ...
              if(iy != iy_old)
                   ...
                   if(iy_old)
                        XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,0,iy_old,
                                  Jgc0.dev_width,iy_old);
                   XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,0,iy,Jgc0.dev_width,iy);
                   }
              if(ix != ix_old)
                   {
                   if(ix_old)
                       XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,ix_old,0,ix_old,
                                 Jgc0.dev_height);
                   XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,ix,0,ix,Jgc0.dev_height);
                   }
...
if(big_crosshair) {
     XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,0,iy_old,Jgc0.dev_width,iy_old);
     XDrawLine(Xgc0.display,Xgc0.win,Xgc0.cursorgc,ix_old,0,ix_old,Jgc0.dev_height);
   }
...
/* Read the current size of the window and resize it if necessary:
   ...
   if(newwidth > 200 && newwidth < 900) Jgc0.dev_width = newwidth;
   if(newheight > 200 && newheight < 900) Jgc0.dev_height = newheight;
   XResizeWindow(Xgc0.display,Xgc0.win,Jgc0.dev_width,Jgc0.dev_height);

*/

/* Copies backup window to exposed window: */
    XCopyArea(Xgc0.display, Xgc0.backing, Xgc0.win, Xgc0.gc, 0, 0, 
              Jgc0.dev_width, Jgc0.dev_height, 0, 0);
...
******************************************************************************/
int JLP_GDev_X11::plot_image(int *image2, int nx2, int ny2, int idim,
...
ny = ny2 * gamma_d;
#ifdef DEBUG
printf("JLP_GDev_X11::plot_image/nx2=%d ny2=%d gamma_d=%d, nx=%d, ny=%d\n",nx2, ny2, gamma_d, nx, ny);
#endif

/* For some unknown reason X11 has problems when numbers which are not
multiple of 4 */
if(nx < 4 || ny < 4) {
  fprintf(stderr,"JLP_GDev_X11::plot_image/Error: too few points for display: nx=%d ny=%d\n",nx,ny);
  return(-1);
  }

if((nx/4)*4 != nx || (ny/4)*4 != ny) {
#ifdef DEBUG
  printf("JLP_GDev_X11::plot_image/Warning, bad image format for display: nx=%d ny=%d\n",nx,ny);
#endif
  nx = (nx/4)*4;
  ny = (ny/4)*4;
  printf("JLP_GDev_X11::plot_image/I correct it to: nx=%d ny=%d\n",nx,ny);
  }
...
*/
if (xi == NULL)  {
      printf("*** JLP_GDev_X11::plot_image/XGetImage error!\n"); 
      printf("*** nx = %d  ny = %d \n",nx,ny); 
      return(-1);}
...
if(x_ul > Jgc0.dev_width || y_ul < 0) {
  printf("JLP_GDev_X11::plot_image/Fatal error: x_ul=%d y_ul=%d\n", x_ul, y_ul);
...
switch (status)
    {
    case BadDrawable:
     printf("JLP_GDev_X11::plot_image/XPutImage/error: bad drawable\n");
     break;
    case BadMatch:
     printf("JLP_GDev_X11::plot_image/XPutImage/error: bad match\n");
     break;
    case BadValue:
     printf("JLP_GDev_X11::plot_image/XPutImage/error: bad value\n");
     break;
    default:
     break;
    }
