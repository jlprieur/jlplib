...
// To define the virtual class JLP_iGDev (Image graphic Devices) 
#include "jlp_igdev.h"

#ifndef __jlp_igd_X11_h                     /* sentry */
#define __jlp_igd_X11_h

#include "jlp_x11_def.h"
#include "jlp_splot_def.h"  //  BIT_PER_VIDEO_PIXEL
...
class JLP_iGDev_X11 : public JLP_iGDev {

public:

/* Constructor for images:
*   JLP_iGDev_X11("display", "Xdisp1", "test.fits", image_f1, 128, 128, 
*                 &gamma1, &gamma_d, &status, 1);     (for images)
*/
 JLP_iGDev_X11(const char *plotdev, const char *out_filename, const char* title, 
       ...
       { 
         int nz1 = 1;
         Xgc0.xvec = NULL;
         status = SetupForImage(this, plotdev, out_filename,
                                title, image_f1, 
                                nx1, ny1, nz1, gamma1, gamma_d, itlk);
       };


// Destructor:
  virtual ~JLP_iGDev_X11() {
  JLP_iGDev_X11::close_device();
  if(italk > 1) printf("JLP_iGDev_X11: destructor has been called\n");
  }

// Close graphic device
  int close() {this->~JLP_iGDev_X11(); return(0);}
  int close_device();
...
// To fill a rectangle (i.e. to paint it with a color):
  int FilledRect1(double x0, double y0, double x1, double y1, 
                  char *color){return(-1);}
...
inline  void conv_mgo_to_dev(const int x, const int y, double& d_x, double& d_y) {
      d_x = (double)x * Jgc0.g_dx;
      d_y = (double)Jgc0.devheight  - (double)y * Jgc0.g_dy;
      }
...
inline  void conv_dev_to_mgo(const double d_x, const double d_y, int& x, int& y) {
      ...
      y = NINT(((double)Jgc0.devheight  - d_y) / Jgc0.g_dy);
      }
