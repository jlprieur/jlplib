...
*
***********************************************************************/
void JLP_Gsegraf::DrawContours3d(int icontour, double *xpoints, 
                                 double *ypoints, double *zpoints)
{
/* Declare variables */
int i, j, ic, nc, nxvalues, nyvalues, nzvalues, flag;
double axis_length, *p, origin[3], Ry[9], Rz[9], Ryz[9];
...
/* Check if contour line goes through polygon */
   if ( zmin_polygon < contour && contour < zmax_polygon )
      ...
      jlp_gseg1->GSEG_DrawLine(points, contourcolors[icontour-1], 1);
      }
   }
