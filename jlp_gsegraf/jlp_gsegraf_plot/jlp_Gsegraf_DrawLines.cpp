...
double line_coords[4], x1, y1, x2, y2, xdata2, ydata2, wx, wy;
...
/* Draw lines between points within plotting area */
      if ( i2 > i1 )
         ...
         ipoints = 0;
         xdata2 = (x1 - xmin) * xscale;
         jlp_gseg_axes1->DrawXPoint(xdata2, &wx);
         points->coords[0] = wx;
         ydata2 = (y1 - ymin) * yscale;
         jlp_gseg_axes1->DrawYPoint(ydata2, &wy);
         points->coords[1] = wy;
         for ( j=i1+1; j<=i2-1; j++ )
            {
            ipoints++;
            xdata2 = (x[j] - xmin) * xscale;
            jlp_gseg_axes1->DrawXPoint(xdata2, &wx);
            points->coords[2*ipoints] = wx;
            ydata2 = (y[j] - ymin) * yscale;
            jlp_gseg_axes1->DrawYPoint(ydata2, &wy);
            points->coords[2*ipoints+1] = wy;
            }
         if ( i2 < npts )
            {
            ipoints++;
            xdata2 = (x2 - xmin) * xscale;
            jlp_gseg_axes1->DrawXPoint(xdata2, &wx);
            points->coords[2*ipoints] = wx;
            ydata2 = (y2 - ymin) * yscale;
            jlp_gseg_axes1->DrawYPoint(ydata2, &wy);
            points->coords[2*ipoints+1] = wy;
            }
      ...
      /* Draw lines between points outside plotting area if lines intersect plotting area */
         ...
         if ( Clip2d(xmin, xmax, ymin, ymax, &line_coords[0]) == 1 )
            ...
            xdata2 = (line_coords[0] - xmin) * xscale;
            jlp_gseg_axes1->DrawXPoint(xdata2, &wx);
            points->coords[0] = wx;
            ydata2 = (line_coords[1] - ymin) * yscale;
            jlp_gseg_axes1->DrawYPoint(ydata2, &wy);
            points = jlp_canvas_points_new(2);
            xdata2 = (line_coords[2] - xmin) * xscale;
            jlp_gseg_axes1->DrawXPoint(xdata2, &wx);
            points->coords[2] = wx;
            ydata2 = (line_coords[3] - ymin) * yscale;
            jlp_gseg_axes1->DrawYPoint(ydata2, &wy);
            points->coords[3] = wy;
