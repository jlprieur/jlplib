...
/*************************************************************************
* Plots 3-dimensional mesh data for truncated polygons.
* Plot the interpolated points (when data points are on the edges)
**************************************************************************/
void JLP_Gsegraf:: PlotInterp3d ( int iplot,
...
/* Interpolate between points 0 and 1 */
   if ( (flag_points[0] == -1 && flag_points[1] >= 0) 
        || (flag_points[0] >= 0 && flag_points[1] == -1) )
      {
      flag_interp_min[0] = 1;
...
/* Interpolate between points 1 and 2 */
   if ( (flag_points[1] == -1 && flag_points[2] >= 0) 
        || (flag_points[1] >= 0 && flag_points[2] == -1) )
      {
      flag_interp_min[1] = 1;
...
/* Interpolate between points 3 and 0 */
   if ( (flag_points[3] == -1 && flag_points[0] >= 0) 
        || (flag_points[3] >= 0 && flag_points[0] == -1) )
      {
      flag_interp_min[3] = 1;
...
/* style_flag= 7 : find polygon minimum and maximum z coordinates */
   if ( style_flag0 == 7 )
      {
...
/* Draw polygon */
// 5="mesh"
//   printf("DEBUG: plot_type=%d style_flag=%d\n", gseg_plot_type0, style_flag0);
