...
{
/* Declare variables */
int i, ifunc, iplot, npts, nplots0, index_stemflags;
unsigned style_size0;
double rmin0, rmax0, xorigin0, yorigin0, radius0, rscale0;
double x, y, theta, r, r1;
char style_char10;
...
/* Draw stem lines */
      ...
      if ( strcmp(&stemflags[index_stemflags],  "on") == 0 ||
         ...
         for ( i=1; i<=npts; i++ )
            {
            theta = jlp_gseg_data1->XData(iplot, i-1);
            r     = jlp_gseg_data1->YData(iplot, i-1);
            if ( r <= rmax0 )
               {
...
/* Draw lines */
      if (style_char10 == 'l' )
         DrawLineSegmentsPolar(iplot, npts, xorigin0, yorigin0, rmin0, rmax0, rscale0, 'l');


/* Draw dashed lines */
      else if (style_char10 == 'd' )
         DrawLineSegmentsPolar(iplot, npts, xorigin0, yorigin0, rmin0, rmax0, rscale0, 'd');


/* Draw dotted lines */
      else if (style_char10 == '.' )
         DrawLineSegmentsPolar(iplot, npts, xorigin0, yorigin0, rmin0, rmax0, rscale0, '.');


/* Draw symbols in symbol_string1 ("cCtTsSiIpPhH") */
      else if ( jlp_gseg_data1->DrawSymbol1OptionFromStyleChar1(iplot, &ifunc) == 0 )
         {
         for ( i=1; i<=npts; i++ )
            {
            theta = jlp_gseg_data1->XData(iplot, i-1);
...
/* Draw symbols in symbol_string2 ("+xra") */
      else if ( jlp_gseg_data1->DrawSymbol2OptionFromStyleChar1(iplot, &ifunc) == 0 )
         {
         for ( i=1; i<=npts; i++ )
           {
           theta = jlp_gseg_data1->XData(iplot, i-1);
...
*
***********************************************************************/
void JLP_Gsegraf::DrawLineSegmentsPolar(int iplot, int npts,
                                        double xorigin0, double yorigin0, 
                                        double rmin0, double rmax0,
                                        double rscale0, int linechar)
{
/* Declare variables */
int idraw, iseg, nseg, npts_seg, iseg1, linebrk;
unsigned style_size0;
UINT32 outline_color_rgba0, fill_color_rgba0;
char linetype[7];
double *xdat, *ydat;
...
/* Draw all line segments except last */
 ...
 iseg1 = 0;
 linebrk = jlp_gseg_data1->Linebreak(iplot, iseg1);
 for (iseg=1; iseg<=nseg; iseg++ ) {
    if(( linebrk > 0) && (linebrk < npts )) {
       idraw++;
// npts_seg : number of points in the segment to be plotted:
       npts_seg = linebrk - iseg1;
       xdat = jlp_gseg_data1->XDataPtr(iplot, iseg1);
       ydat = jlp_gseg_data1->YDataPtr(iplot, iseg1);
...
/* Draw last line segment */
 if ( idraw > 0 ) {
    ...
    DrawLinesPolar(npts_seg, xdat, ydat, xorigin0, yorigin0, 
                   rmin0, rmax0, rscale0, fill_color_rgba0, 
                   style_size0, linetype);
/* Draw continuous line */
   } else {
     xdat = jlp_gseg_data1->XDataPtr(iplot, 0);
     ydat = jlp_gseg_data1->YDataPtr(iplot, 0);
