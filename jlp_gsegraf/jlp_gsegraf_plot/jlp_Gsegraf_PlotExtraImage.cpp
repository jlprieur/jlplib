...
*
* Draws an additional image from image file.
*
...
*
*******************************************************************************/
void JLP_Gsegraf::PlotExtraImage ()
{
/* Declare variables */
int status; 
...
// Get image plot settings: 
 status = jlp_gseg_data1->GetExtraImagePlotSettings(image_filename,
                                                    image_coords_flag, 
                                                    text_image_anchor,
...
/* Check image is within plot box for absolute coordinates */
  if (!( strcmp(image_coords_flag, "abs") == 0 &&
               (xanchor < 0 || yanchor < 0) ))
  status = jlp_gseg1->GSEG_PlotExtraImage(image_filename, xanchor, yanchor, 
                                     text_image_anchor);
 if (status != 0 ) 
  fprintf(stderr, "PlotExtraImage/GSEG_PlotExtraImage/error plotting %s status=%d\n", 
