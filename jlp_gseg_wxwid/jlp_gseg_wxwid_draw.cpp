...
*
*************************************************************************/
void JLP_Gseg_Wxwid::GSEG_Clear()
{
if(gseg_backup_dc != NULL) {
  gseg_backup_dc->Clear();
 }
}
/*************************************************************************
...
/* DEBUG
printf("GSEG_SetWindowLimits/ x_scale=%f yscale=%f\n", 
...
/*************************************************************************
* Conversion of coordinates from wxwidgets screen coords to postscript coords.
*
* INPUT:
*  xi, yi
*
...
*xo = (double)(xi - xx1) * x_wx_to_pst_scale1 + pst_xx1;
*yo = (double)(yi - yy1) * y_wx_to_pst_scale1 + pst_yy1;

/* DEBUG
printf("jlp_convert_wx_to_pst/ x_scale=%f y_scale=%f and %d %d %d %d\n", 
        x_wx_to_pst_scale1, y_wx_to_pst_scale1, xi, yi, *xo, *yo);
*/
return(0);
}
/**************************************************************************
...
// Transparent brush:
// NB: transparent brush when alpha is 00, opaque when alpha is FF
...
// Transparent brush when alpha is 00, opaque whan alpha is FF:
fill_color0 = 0xFFFFFF00;
jlp_set_brush_color(fill_color0);
...
if(gseg_backup_pst_dc != NULL) {
 ...
 for(i = 0; i < npts; i++) {
   jlp_convert_wx_to_pst(points1[i].x, points1[i].y, &(points_1[i].x), 
                         &(points_1[i].y), &in_frame);
   }
 gseg_backup_pst_dc->DrawPolygon(npts, points_1, 0, 0, wxODDEVEN_RULE);
// Free memory
 delete[] points_1;
 }
...
// Called with transparent brush when alpha is 00, opaque when alpha is FF:
 jlp_set_brush_color(fill_color_rgba0);
...
/* Draw rectangle */
// JLP2017: I add line_width to correct a bug 
// when drawing the axis box in "jlp_GsegAxes_DrawAxisLabels.cpp" 
 width0 = x2 - x1 + line_width0; 
 height0 = y2 - y1 + line_width0;
...
if(gseg_backup_pst_dc != NULL) {
   jlp_convert_wx_to_pst(x1, y1, &x_1, &y_1, &in_frame);
   jlp_convert_wx_to_pst(x2, y2, &x_2, &y_2, &in_frame);
   width0 = x_2 - x_1 + line_width0; 
   height0 = y_2 - y_1 + line_width0;
...
if(gseg_backup_pst_dc != NULL) {
   ...
   gseg_backup_pst_dc->DrawCircle(x_0, y_0, diameter_0 / 2);
  }
return;
}
/************************************************************************
