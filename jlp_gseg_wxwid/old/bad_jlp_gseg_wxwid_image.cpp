...
// Postscript:
   if(gseg_backup_pst_dc != NULL) {
    ...
    gseg_backup_pst_dc->DrawBitmap(*my_bitmap, xx_0, yy_0, true);
   }
...
*  x0, y0 : position
*****************************************************************************/
int JLP_Gseg_Wxwid::GSEG_DrawImage(const char *image_filename, double x0, 
                                   double y0, const char *text_anchor0)
{
...
// and then a bitmap
  ...
  if(!my_image0->Ok()) {
    fprintf(stderr, "GSEG_DrawBackgroundImage/Error opening %s\n", 
            image_filename);
    return(-1);
    }
...
// Postscript:
   if(gseg_backup_pst_dc != NULL) {
    jlp_convert_wx_to_pst(xx0, yy0, &xx_0, &yy_0, &in_frame);
    gseg_backup_pst_dc->DrawBitmap(*my_bitmap0, xx_0, yy_0, true);
   }
