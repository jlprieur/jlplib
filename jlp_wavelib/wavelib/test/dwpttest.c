...
int main() {
	...
	wpt_object wt;
	double *inp, *oup, *diff;
	char name1[32], per_str[16], logenergy_str[16];
        strcpy(name1, "db4");
        strcpy(per_str, "per");
        strcpy(logenergy_str, "logenergy");
	...
	setDWPTExtension(wt, per_str);// Options are "per" and "sym". Symmetric is the default option
	setDWPTEntropy(wt, logenergy_str, 0);
